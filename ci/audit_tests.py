...
SOURCE_ROOT = REPO_ROOT / "dataengineer"
        ...
        if _PATCH_BUILTINS_OPEN.search(line) and not noqa_match and rule_applies("patch_builtins_open", tier):
            issues.append(
                Issue(
                    ...
                    quote=stripped[:120],
                    suggestion="Patch the specific module path, e.g. `patch('dataengineer.module.path.open')`, not builtins.",
                    tier=tier,
                )
            )
...
def check_missing_test_for_module() -> list[Issue]:
    """Unit-tier only: dataengineer/X/Y.py exists but no test file imports it."""
    imported_modules: set[str] = set()
    import_pattern = re.compile(r"^\s*(?:from|import)\s+(dataengineer(?:\.[A-Za-z_][A-Za-z0-9_]*)*)")
    ...
    for py in SOURCE_ROOT.rglob("*.py"):
        if py.name == "__init__.py":
            continue
        mod_parts = ("dataengineer",) + py.relative_to(SOURCE_ROOT).with_suffix("").parts
