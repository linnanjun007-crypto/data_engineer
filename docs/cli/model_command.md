...
All selections are persisted to the project-level `.dataengineer/config.yml` so they survive session restarts and are scoped to the current working directory.
...
### Project Override (`.dataengineer/config.yml`)

When you select a model via `/model`, the choice is saved to `.dataengineer/config.yml` in the current working directory:
...
1. **Project override** — `.dataengineer/config.yml` in the current working directory
