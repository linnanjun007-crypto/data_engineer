...
所有选择都会持久化到项目级 `.dataengineer/config.yml`，在会话重启后仍然生效，并且作用域限定在当前工作目录。
...
### 项目覆盖 (`.dataengineer/config.yml`)

当你通过 `/model` 选择模型时，选择会保存到当前工作目录的 `.dataengineer/config.yml`：
...
1. **项目覆盖** — 当前工作目录中的 `.dataengineer/config.yml`
