...
## 1. Prerequisites: Configure Your DataEngineer Agent
...
This sets up your LLM provider (API key) and saves the configuration to `~/.dataengineer/conf/agent.yml`. The tutorial will automatically configure the california_schools database for you.
...
```text
Welcome to DataEngineer tutorial 🎉
...
dataengineer-agent bootstrap-kb \
  --config ~/.dataengineer/conf/agent.yml \
...
dataengineer-agent bootstrap-kb \
  ...
  --components metrics \
  --kb_update_strategy overwrite \
  --success_story ~/.dataengineer/benchmark/california_schools/success_story.csv \
...
dataengineer-agent bootstrap-kb \
  ...
  --kb_update_strategy overwrite \
  --sql_dir ~/.dataengineer/benchmark/california_schools/reference_sql \
...
```bash
dataengineer-agent benchmark   --database california_schools   --benchmark california_schools   --workflow datus_schools
```

Save the results:

```bash
dataengineer-agent eval   --database california_schools   --benchmark california_schools   --output_file schools1.txt
```
...
```bash
dataengineer-agent benchmark   --database california_schools   --benchmark california_schools   --workflow datus_schools_context
```

Save the results:

```bash
dataengineer-agent eval   --database california_schools   --benchmark california_schools   --output_file schools2.txt
```
