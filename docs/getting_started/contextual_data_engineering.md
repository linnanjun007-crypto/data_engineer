...
- In [DataEngineer CLI](../cli/introduction.md), you can browse and edit them via `/catalog` and `/subject`
- Use `dataengineer-agent bootstrap-kb` to batch-initialize and cold-start the knowledge base
...
```bash
dataengineer-agent --database schools --web
```
