...
DAComp is **not bundled** with `dataengineer-agent`. Download the dataset from Hugging Face first:
...
export SUPERSET_SECRET_KEY='dataengineer-test-secret-key-not-for-prod'
...
Add the following minimum configuration to your `~/.dataengineer/conf/agent.yml`:
...
```bash
cd "$DACOMP_HOME"
dataengineer-cli --database lever_duckdb
```
