...
几分钟内即可上手 DataEngineer Agent。本指南将带你完成安装、配置和首次体验。
...
=== "测试版"
    ...
    ```bash
    pip install --no-deps -i https://test.pypi.org/simple/ dataengineer-agent
    ```
...
!!! tip "演示数据库"
    ...
    **连接字符串：** `~/.dataengineer/sample/duckdb-demo.duckdb`

配置会写入 `~/.dataengineer/conf/agent.yml`。后续如果你想追加模型或数据库，重新运行 `dataengineer-agent configure` 即可。
...
!!! note
    `codex` 目前虽然仍出现在 provider catalog 中，但现阶段 `dataengineer-agent configure` 还不能稳定完成它的完整初始化流程，因此这里不把它视为当前可用的交互式配置选项。
...
`dataengineer-agent init` 会读取你已经通过 `dataengineer-agent configure` 保存的默认模型和数据库配置，扫描当前目录，并生成项目级 `AGENTS.md`。

如需把某个已配置数据库的表结构摘要一并写入 `AGENTS.md`，可额外指定：

```bash
dataengineer-agent init --database duckdb-demo
```

## 步骤 2：启动 DataEngineer CLI

使用已配置的数据库启动 DataEngineer CLI：

!!! tip "配置提示"
    你可以随时通过 `dataengineer-agent configure` 继续添加模型或数据库。详见我们的[配置指南](../configuration/introduction.md)。
...
??? example "详细分析输出"
    ...
    |                                                    Action Stream                                                                      |
    +=======================================================================================================================================+
    | 🔧 Tool call - glob('{"pattern": "*", "path": "/Users/yt/.dataengineer/workspace"}') - ✓                                                    |
    |     No matches found (0.5s)                                                                                                          |
    | 💬 Thinking: Let me check the directory structure to see what's available:                                                           |
    | 🔧 Tool call - glob('{"pattern": "**/*", "path": "/Users/yt/.dataengineer/workspace"}') - ✓ (0.5s)                                          |
