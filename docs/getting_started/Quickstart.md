...
Get started with DataEngineer Agent in just a few minutes. This guide will walk you through installation, setup, and your first interactions with Datus.
...
curl -fsSL https://raw.githubusercontent.com/datus-ai/datus-agent/main/install-dev.sh | DATAENGINEER_REF=feature/foo sh
```

Both scripts bootstrap `uv`, create a dedicated venv at `~/.dataengineer/venv` (Python 3.12 is downloaded automatically if missing), and write `datus`, `dataengineer-cli`, `dataengineer-api`, `dataengineer-mcp`, `dataengineer-agent`, `dataengineer-gateway`, and `dataengineer-pip` shims into `~/.local/bin`. Open a new shell (or `source ~/.zshrc`) so the new PATH takes effect.

To install additional Python packages into the global venv later, use:

```bash
dataengineer-pip install <package>
# equivalent to ~/.dataengineer/venv/bin/pip install <package>
```

Pin a released version with the stable installer — note the variable is passed to the receiving shell, not to `curl`:

```bash
curl -fsSL https://raw.githubusercontent.com/datus-ai/datus-agent/main/install.sh | DATAENGINEER_VERSION=0.2.6 sh
```

Other variables supported by both scripts: `DATAENGINEER_HOME`, `DATAENGINEER_BIN_DIR`, `DATAENGINEER_FORCE=1`, `DATUS_NO_MODIFY_PATH=1`.
...
=== "virtualenv"
    ...
    source dataengineer/bin/activate
    ```
...
=== "Beta Release"
    ...
    ```bash
    pip install -i https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple/ dataengineer-agent
    ```
...
!!! tip "Demo Database"
    ...
    **Connection string:** `~/.dataengineer/sample/duckdb-demo.duckdb`

Database adapter plugins (Snowflake, MySQL, PostgreSQL, StarRocks) are auto-installed when selected.

Configuration is saved to `~/.dataengineer/conf/agent.yml`. Run `dataengineer-agent configure` again anytime to add more models or databases.
...
!!! note
    `codex` still appears in the provider catalog, but the current `dataengineer-agent configure` flow is not ready to set it up end-to-end yet. Do not rely on it as a working interactive wizard option for now.
...
```bash
dataengineer-agent init --database demo
```

## Step 2: Launch DataEngineer CLI

Start the DataEngineer CLI with your configured database:

!!! tip "Multiple Databases"
    You can add more databases anytime with `dataengineer-agent configure`. Use `dataengineer-agent service list` to see all configured databases.

```bash title="Terminal"
dataengineer-cli --database duckdb-demo
```
```{ .yaml .no-copy }
...
??? example "Detailed Analysis Output"
    ...
    |                                                    Action Stream                                                                      |
    +=======================================================================================================================================+
    | 🔧 Tool call - glob('{"pattern": "*", "path": "~/.dataengineer/workspace"}') - ✓                                                    |
    |     No matches found (0.5s)                                                                                                          |
    | 💬 Thinking: Let me check the directory structure to see what's available:                                                           |
    | 🔧 Tool call - glob('{"pattern": "**/*", "path": "~/.dataengineer/workspace"}') - ✓ (0.5s)                                          |
