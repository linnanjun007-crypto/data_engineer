...
Add the following configuration to your `~/.dataengineer/agent.yml`:
...
!!! tip
    You can also add the database interactively with `dataengineer-agent configure` instead of editing YAML manually.
...
```bash
dataengineer-agent bootstrap-bi --database superset
```
