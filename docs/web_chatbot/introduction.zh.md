...
Datus Web chatbot 提供一个易用的网页界面，用于与 Datus AI 助手交互。它通过 FastAPI 服务端托管 React 前端组件（`@dataengineer/web-chatbot`），面向自然语言转 SQL 的场景，用户无需掌握命令行即可完成对话式分析。
...
3. 查询会保存到 `~/.dataengineer/benchmark/[subagent]/success_story.csv`
