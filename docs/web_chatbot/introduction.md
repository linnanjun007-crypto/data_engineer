...
The Datus Web Chatbot provides a user-friendly web interface for interacting with the Datus AI assistant. It serves a React-based frontend (`@dataengineer/web-chatbot`) backed by the DataEngineer Agent FastAPI server, offering an intuitive chat experience for natural language to SQL conversion without requiring command-line knowledge.
...
3. The query is saved to `~/.dataengineer/benchmark/[subagent]/success_story.csv`
