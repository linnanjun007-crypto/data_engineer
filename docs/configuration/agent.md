...
The agent configuration defines the core settings for your DataEngineer Agent, including the target model selection and all available LLM providers that can be used throughout the system.
...
The active model is determined by project-level override (`.dataengineer/config.yml`) or the base `agent.target` field. The `/model` command writes to the project override automatically.

#### Project-Level Override (`.dataengineer/config.yml`)

The `/model` command persists selections to `.dataengineer/config.yml` in the current working directory:
...
Resolution order: `.dataengineer/config.yml` → `agent.target`.
...
- The active provider/model is stored in `.dataengineer/config.yml` (written by `/model`)
...
```yaml title=".dataengineer/config.yml"
target:
  provider: openai
  model: gpt-4.1
