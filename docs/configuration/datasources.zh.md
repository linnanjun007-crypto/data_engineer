...
DataEngineer Agent 在 `agent.yml` 中通过 `agent.services` 统一管理运行时集成。本页聚焦 `services.datasources` 下的数据库连接；语义层、BI 平台和调度器分别在同级页面单独说明。
...
使用 `dataengineer-agent configure` 交互式添加、删除或管理数据库：
