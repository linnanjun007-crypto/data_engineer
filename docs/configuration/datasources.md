...
The runtime services in DataEngineer Agent live under `agent.services` in `agent.yml`. This page focuses on database connections in `services.datasources`. Semantic adapters, BI platforms, and schedulers are documented on their sibling pages.
...
- **Plugin Adapters**: Install additional database adapters via `dataengineer-agent configure`
...
Use `dataengineer-agent configure` to add, delete, or manage databases interactively:
...
```bash
# List all databases
dataengineer-agent service list

# Add a database interactively
dataengineer-agent service add

# Delete a database interactively
dataengineer-agent service delete
```

### Specify a Custom Config

```bash
dataengineer-agent service list --config /path/to/agent.yml
dataengineer-agent configure --config /path/to/agent.yml
```
...
```bash
dataengineer-cli --database my_duckdb
dataengineer-agent run --database my_snowflake --task "..." --task_db_name ANALYTICS
```
