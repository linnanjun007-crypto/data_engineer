...
Configure storage settings for DataEngineer Agent's embedding models and vector databases. The storage configuration manages how metadata, documents, and metrics are embedded and stored for efficient retrieval during schema linking and knowledge search.
...
- `data/dataengineer_db_<datasource_name>` for each configured datasource
- Example: `data/dataengineer_db_snowflake`, `data/dataengineer_db_local_sqlite`
...
=== "Enterprise Quality Setup"
    ...
    ```yaml
    storage:
      base_path: /opt/dataengineer/embeddings
...
DataEngineer Agent uses a dual-track storage architecture — **Vector DB** for embeddings and **RDB** for structured metadata. Both support pluggable backends via a registry + entry-point mechanism.

> For an architectural overview, see [Knowledge Base Introduction](../knowledge_base/introduction.md#storage-backends).

### Default Backend

By default (no `rdb` / `vector` section in YAML), DataEngineer Agent uses:

- **Vector**: LanceDB (file-based, zero-config)
- **RDB**: SQLite (file-based, zero-config)

Data is stored under `data/dataengineer_db_<datasource>/`.
