...
```bash
# CLI 模式
dataengineer-cli --datasource spidersnow --save_llm_trace

# 主程序模式
python dataengineer/main.py run --datasource bird_sqlite --task "Query all users" --task_db_name test.db --save_llm_trace

# Benchmark 模式
python dataengineer/main.py benchmark --datasource bird_sqlite --benchmark bird_dev --save_llm_trace
```
...
```bash
# 在开发期间启用跟踪以调试提示词
dataengineer-cli --datasource local_duckdb --save_llm_trace
```
...
```bash
# 记录所有 LLM 交互以进行审计
python dataengineer/main.py run --datasource prod --task "sensitive query" --save_llm_trace
```
...
python dataengineer/main.py run --datasource bird_sqlite --task "Complex query" --save_llm_trace
```
