...
1. **Skill discovery** — The system scans `skills.directories` (or defaults: `~/.dataengineer/skills`, `./skills`) to find all `SKILL.md` files.
...
!!! note
    If no global `skills:` section is present in `agent.yml`, the system automatically creates a default skill manager that scans `~/.dataengineer/skills` and `./skills`.
...
Tokens are saved at `~/.dataengineer/marketplace_auth.json` and automatically included in all subsequent marketplace requests. Tokens expire after 24 hours; re-run `login` to refresh.
...
skills:
  ...
  install_dir: "~/.dataengineer/skills"            # Where marketplace skills are installed
```
...
2. Extracts to `~/.dataengineer/skills/<name>/`
