...
Memory files are stored under `.dataengineer/memory/` in the workspace, with each agent having its own subdirectory:
...
```bash
cat {workspace_root}/.dataengineer/memory/chat/MEMORY.md
```
...
```text
~/my_project/.dataengineer/memory/chat/MEMORY.md
```
