...
1. **技能发现** — 系统扫描 `skills.directories`（或默认路径：`~/.dataengineer/skills`、`./skills`）查找所有 `SKILL.md` 文件。
...
!!! note
    如果 `agent.yml` 中没有全局 `skills:` 部分，系统会自动创建默认的技能管理器，扫描 `~/.dataengineer/skills` 和 `./skills`。
...
令牌保存在 `~/.dataengineer/marketplace_auth.json`，并自动包含在所有后续市场请求中。令牌在 24 小时后过期；重新运行 `login` 以刷新。
...
skills:
  ...
  install_dir: "~/.dataengineer/skills"            # 市场技能的安装目录
```
...
2. 解压到 `~/.dataengineer/skills/<name>/`
