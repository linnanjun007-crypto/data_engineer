...
Writes configuration to `~/.dataengineer/conf/agent.yml` using the `services.datasources` format.
...
**Repeatable:** Yes. If `~/.dataengineer/conf/agent.yml` already exists, you'll be asked to confirm overwrite. The existing configuration is fully replaced.

**Output:** `~/.dataengineer/conf/agent.yml` with structure:
```yaml
agent:
  providers:
    openai:
  ...
  services:
    ...
    bi_platforms: {}
    schedulers: {}
  project_root: ~/.dataengineer/workspace
```
...
**What it does:**
1. Loads LLM and service configuration from `~/.dataengineer/conf/agent.yml`
