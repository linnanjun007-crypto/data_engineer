...
本指南将引导你完成飞书机器人的配置，通过 DataEngineer Gateway IM 网关连接到 DataEngineer Agent。飞书使用官方 `lark-oapi` SDK 的 **WebSocket 长连接**，机器人主动发起出站连接 —— 无需公网 URL 或 Webhook 端点。

## 前置条件

- 你有开发者权限的飞书租户（组织）
- 已安装并配置好 DataEngineer Agent
...
3. 点击**创建企业自建应用**。
4. 填写应用名称（如 "DataEngineer Agent"）和描述。
5. 点击**创建**。
...
DataEngineer Gateway 使用飞书 WebSocket 长连接接收事件，无需公网回调 URL。

1. 在左侧导航栏，进入**事件与回调**。
2. 在**订阅方式**中，选择**使用长连接接收事件**（WebSocket 模式）。

    !!! tip "为什么选择 WebSocket 模式？"
        WebSocket 模式非常适合 DataEngineer Gateway，因为它不需要公网可达的服务器。机器人主动发起到飞书服务器的 WebSocket 出站连接，事件通过该连接推送。
