...
DataEngineer Gateway is the IM (Instant Messaging) gateway module for DataEngineer Agent. It connects Datus to chat platforms such as Slack and Feishu (Lark), enabling users to interact with the data analysis agent directly from their team communication tools.

## Key Design Principles

- **Outbound long connections only** — DataEngineer Gateway actively connects to each IM platform using WebSocket, Socket Mode, or Stream SDK. No webhook endpoint or public URL is needed.
...
You only need to install the SDK(s) for the platform(s) you use. DataEngineer Gateway will raise a clear error if a required SDK is missing.
...
```bash
dataengineer-gateway --config conf/agent.yml

# Or via uv
uv run dataengineer-gateway --config conf/agent.yml
```
...
```bash
# Start in background
dataengineer-gateway --daemon

# Check status
dataengineer-gateway --action status

# Stop
dataengineer-gateway --action stop

# Restart
dataengineer-gateway --action restart
```

All daemon commands also work with `uv run`, e.g. `uv run dataengineer-gateway --daemon`.

By default, the PID file is stored at `~/.dataengineer/run/dataengineer-gateway.pid` and daemon logs are written to `logs/dataengineer-gateway.log`. You can override these paths:

```bash
dataengineer-gateway --daemon --pid-file /var/run/dataengineer-gateway.pid --daemon-log-file /var/log/dataengineer-gateway.log
```
...
| `--datasource` | `default` (or `DATAENGINEER_DATASOURCE` env) | Default datasource for all channels |
| `--host` | `0.0.0.0` | Health-check server bind host |
| `--port` | `9000` | Health-check server bind port |
| `--debug` | `false` | Enable debug logging |
| `--log-level` | `INFO` (or `DATAENGINEER_LOG_LEVEL` env) | Log level: DEBUG, INFO, WARNING, ERROR, CRITICAL |
| `--daemon` | `false` | Run in background as a daemon |
| `--action` | `start` | Daemon action: `start`, `stop`, `restart`, `status` |
| `--pid-file` | `~/.dataengineer/run/dataengineer-gateway.pid` | PID file path |
| `--daemon-log-file` | `logs/dataengineer-gateway.log` | Daemon log file path |

## Features

### Real-Time Streaming

DataEngineer Gateway streams every stage of the agent's processing back to the chat:
...
DataEngineer Gateway provides built-in slash commands that are intercepted before messages reach the agentic loop. You can type them with or without the `/` prefix.
...
```bash
dataengineer-gateway --config conf/agent.yml --debug
```

Or set the environment variable:

```bash
export DATAENGINEER_LOG_LEVEL=DEBUG
```
