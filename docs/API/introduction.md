...
management as HTTP endpoints. It is started via the `dataengineer-api` command.

The HTTP service shares the same configuration, knowledge base, and agent capabilities as the `datus` CLI and the
`dataengineer-mcp` MCP server — three entry points backed by one engine.

| Entry point | Best for |
|-------------|----------|
| **CLI** (`datus`) | Local interactive development |
| **MCP** (`dataengineer-mcp`) | Embedding tools in another agent |
| **REST API** (`dataengineer-api`) | Web frontends, services, automation |
...
Datasource isolation is controlled separately by the `--database` CLI flag (or `DATAENGINEER_DATASOURCE` env var) and selects
