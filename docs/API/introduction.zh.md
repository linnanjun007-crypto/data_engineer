...
对外暴露。通过 `dataengineer-api` 命令启动。

API 服务与 `datus` CLI、`dataengineer-mcp` MCP 服务共享同一套配置、知识库与 Agent 能力 — 三种入口,同一个引擎。

| 入口 | 适用场景 |
|------|----------|
| **CLI**(`datus`) | 本地交互式开发 |
| **MCP**(`dataengineer-mcp`) | 嵌入到其他 Agent 中 |
| **REST API**(`dataengineer-api`) | Web 前端、服务化、自动化 |
...
Datasource 隔离由 `--datasource` CLI 参数(或 `DATAENGINEER_DATASOURCE` 环境变量)单独控制,决定加载 `agent.yml`
