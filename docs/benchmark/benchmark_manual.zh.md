...
使用行业标准数据集评估 DataEngineer Agent 的表现与能力。针对 BIRD 与 Spider 2.0-Snow 等数据集运行全面测试，衡量正确率、执行成功率与查询生成质量。
...
将 BIRD dev 数据集下载并解压到 Datus 主目录（默认为 `~/.dataengineer`）：
...
```bash title="Terminal"
dataengineer-agent bootstrap-kb --datasource bird_sqlite --benchmark bird_dev
```
...
- **comparison_results**: 比较结果
  - `actual_sql`: 由 DataEngineer Agent 生成的 SQL
  - `actual_path`: 由 DataEngineer Agent 生成的 SQL 执行后的结果
  - `gold_sql`: 标准答案 SQL
  - `gold_path`: 标准答案的结果
  - `comparison`: 对比结果：
    - `match_rate`: 匹配度
    ...
    - `actual_tables`: DataEngineer Agent 生成结果用到的表
...
| `--config`              | 否   | `~/.dataengineer/conf/agent.yml` 或 `conf/agent.yml`   | Agent 配置文件路径                    |
