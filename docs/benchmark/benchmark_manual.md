...
Evaluate DataEngineer Agent's performance and capabilities using industry-standard benchmarks. Run comprehensive tests against datasets like BIRD and Spider 2.0-Snow to assess accuracy, execution success rate, and query generation quality.

## Overview

DataEngineer Agent benchmark mode enables you to:
...
### Step 1: Install DataEngineer Agent
...
Download and extract the BIRD dev dataset into the DataEngineer home directory (`~/.dataengineer` by default):
...
```bash title="Terminal"
dataengineer-agent bootstrap-kb --database bird_sqlite --benchmark bird_dev
```
...
────────────────────────────────────────────────────────────────────────────────
 DataEngineer Evaluation Summary (Total: 30 Queries)
────────────────────────────────────────────────────────────────────────────────
 ✅ Passed:                                 19    (63%)
      ...
      "comparison_results": [
        {
          ...
          "actual_path": "<USER_HOME>/.dataengineer/save/bird_school/0.csv",
          "gold_path": "<USER_HOME>/.dataengineer/benchmark/california_schools/california_schools.csv",
          "comparison": {
            "match_rate": 1.0,
            "matched_columns": [
              [
...
- **comparison_results**: Comparison results for each task
    - `actual_sql`: SQL generated by DataEngineer Agent
    - `actual_path`: Executed result produced by DataEngineer Agent
...
```bash
dataengineer-agent benchmark --database california_schools --benchmark california_schools --benchmark_task_ids 0 1 2 --workflow <your workflow>
```
👉 See [Step 4: Run Benchmark Tests](#step-4-run-benchmark-tests)

```bash
dataengineer-agent eval --database california_schools --benchmark california_schools --task_ids 0 1 2 
```
...
=== "dataengineer-agent subcommand"
    ```bash title="Terminal"
    dataengineer-agent multi-round-benchmark \
      --database bird_sqlite \
      --benchmark bird_dev \
...
=== "Python module"
    ```bash title="Terminal"
    python -m dataengineer.multi_round_benchmark \
      --database bird_sqlite \
      --benchmark bird_dev \
      --workflow chat_agentic \
      --round 4 \
...
| `--config`              | No       | `~/.dataengineer/conf/agent.yml` or `conf/agent.yml` | Path to agent config file                                            |
