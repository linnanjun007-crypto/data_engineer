...
**内置 Subagent**  是集成在 DataEngineer Agent 系统中的专用 AI 助手。每个subagent专注于数据工程自动化的特定方面——分析 SQL、生成语义模型、将查询转换为可复用指标——共同构成从原始 SQL 到具备知识感知的数据产品的闭环工作流。
...
- **系统提示**：内置模板版本 1.0
- **工作空间**：`~/.dataengineer/data/{datasource}/` 及 subagent 特定子目录
...
使用 `datus --database <datasource>` 启动 DataEngineer CLI，然后使用subagent命令：
...
使用 `datus --database <datasource>` 启动 DataEngineer CLI，然后使用指标生成subagent：
...
`gen_skill` subagent 用于引导用户创建或优化 DataEngineer skill。它可以检查现有 skill、脚手架新的 skill 目录、编辑 `SKILL.md`、执行校验，并检索历史会话中的使用模式。
