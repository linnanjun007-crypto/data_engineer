...
你可以使用 `dataengineer-agent bootstrap-kb` 命令构建：

```bash
dataengineer-agent bootstrap-kb --datasource <your_datasource> --kb_update_strategy [check/overwrite/incremental]
```
...
### 检查当前状态
```bash
dataengineer-agent bootstrap-kb --datasource <your_datasource> --kb_update_strategy check
```

### 完全重建
```bash
dataengineer-agent bootstrap-kb --datasource <your_datasource> --kb_update_strategy overwrite
```

### 增量更新
```bash
dataengineer-agent bootstrap-kb --datasource <your_datasource> --kb_update_strategy incremental
```
