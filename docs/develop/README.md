...
```bash
conda create -n dataengineer-agent python=3.12
conda activate dataengineer-agent
...
```bash
virtualenv dataengineer-agent --python=python3.12
source dataengineer-agent/bin/activate
pip install -r requirements.txt
```
...
LANGSMITH_PROJECT=DataEngineer-agent
```
...
```bash
python -m dataengineer.main probe-llm
```
...
```bash
python -m dataengineer.main check-db --database local_duckdb
```
...
```bash
python -m dataengineer.cli.main --database local_duckdb --config conf/agent.yml

DataEngineer> select * from tree;
┏━━━━━━━━━━━┳━━━━━━━━━━━━━┳━━━━━━━━━━━━┳━━━━━━━━┓
...
```bash
python -m dataengineer.main bootstrap-kb --database spider-snow --benchmark spider2 --kb_update_strategy overwrite
```
...
```bash
python -m dataengineer.main benchmark --database spider-snow --benchmark spider2 --benchmark_task_ids sf_bq104
```

```bash
python -m dataengineer.cli.main --database spider-snow  --config conf/agent.yml

DataEngineer> !darun_screen
...
```bash
python -m dataengineer.main bootstrap-kb --database bird_sqlite --benchmark bird_dev --kb_update_strategy overwrite
```

### Run Tests

```bash
python -m dataengineer.main benchmark --database bird_sqlite --benchmark bird_dev --plan fixed --schema_linking_rate medium --benchmark_task_ids 14 15
```

```bash
python -m dataengineer.main benchmark --database bird_sqlite --benchmark bird_dev --schema_linking_rate fast --benchmark_task_ids 32
```

```bash
python -m dataengineer.main benchmark --database bird_sqlite --benchmark bird_dev --plan fixed --schema_linking_rate medium
```

### Using cli to develop

```bash
python -m dataengineer.cli.main --database bird_sqlite  --config conf/agent.yml
```
...
```bash
python -m dataengineer.main bootstrap-kb --database duckdb --components metrics --kb_update_strategy overwrite
```

### Run Tests

```bash
python -m dataengineer.main benchmark --database duckdb --benchmark semantic_layer --plan metric_to_sql
```
