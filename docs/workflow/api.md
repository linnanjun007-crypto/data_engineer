...
The DataEngineer Agent Workflow API Service is a RESTful API that exposes the power of DataEngineer Agent's natural language to SQL capabilities through HTTP endpoints. This service enables applications to integrate intelligent SQL generation, execution, and workflow management into their systems.
...
python -m dataengineer.api.main --host 0.0.0.0 --port 8000

# Start with multiple workers
python -m dataengineer.api.main --workers 4 --port 8000

# Start in daemon mode (background)
python -m dataengineer.api.main --daemon --port 8000
```
...
```bash
python -m dataengineer.api.main \
  --host 0.0.0.0 \
  --port 8000 \
  --workers 4 \
  --reload \
