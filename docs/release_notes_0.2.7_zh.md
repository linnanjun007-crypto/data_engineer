# DataEngineer-Agent 0.2.7 Release Note（中文版）
...
1. **新增独立 REST API 服务（`dataengineer-api`）** —— 从 Streamlit 后端抽出，可独立部署。
...
| 能力 | PR | 说明 |
|---|---|---|
| 新增 `dataengineer-api` CLI 入口与 FastAPI 模块 | [#520](https://github.com/Datus-ai/Datus-agent/pull/520) | 独立的 REST API，包含 agents / chat / database / explorer / KB / MCP 路由；消除对 `datus_backend` 的 ORM 依赖；可独立部署 |
...
### 2.3 DataEngineer Gateway IM 网关
...
2. **Global 存储按 project 分片**
   - `~/.dataengineer/sessions/{project_name}/{session_id}.db`
   - `~/.dataengineer/data/{project_name}/dataengineer_db/`
...
4. **访问区分级**（[#588](https://github.com/Datus-ai/Datus-agent/pull/588)）：`PathZone` = `INTERNAL` / `WHITELIST`（`~/.dataengineer/skills`、`.dataengineer/skills`、`.dataengineer/memory/{current_node}`）/ `HIDDEN`（`.datus` 其他内容）/ `EXTERNAL`（项目根以外）。
...
uv sync
# 或 pip 安装方式
pip install -U dataengineer-agent
```
...
| `~/.dataengineer/sessions/` | 扁平 | 按 `{project_name}` 分片 | 旧 session 文件移动到 `~/.dataengineer/sessions/{project_name}/` 下即可复用 |
| `~/.dataengineer/data/dataengineer_db/` | 全局 | `~/.dataengineer/data/{project_name}/dataengineer_db/` | 同上 |
...
```bash
# 启动独立 API
dataengineer-api server --host 0.0.0.0 --port 8080 --stream --interactive

# CLI print mode + proxy tools
datus chat --print --proxy_tools query_sql --resume <session_id>

# DataEngineer Gateway 网关以 daemon 启动
dataengineer-gateway --daemon --action start --pid-file ~/.dataengineer/run/gateway.pid
