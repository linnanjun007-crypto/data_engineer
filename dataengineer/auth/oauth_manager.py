...
from dataengineer.auth.oauth_config import (
    AUTHORIZE_URL,
    CALLBACK_PORT,
    CLIENT_ID,
    HTTP_TIMEOUT,
...
from dataengineer.auth.pkce import generate_pkce_pair, generate_state
...
class OAuthManager:
    ...
    def login_browser(self) -> dict:
        ...
        if result["error"]:
            raise DataEngineerException(ErrorCode.OAUTH_AUTH_FAILED, message_args={"error_detail": result["error"]})
        if not result["code"]:
            raise DataEngineerException(
                ErrorCode.OAUTH_AUTH_FAILED,
    ...
    def get_access_token(self) -> str:
        ...
        Raises:
            DataEngineerException: If not authenticated or refresh fails.
        """
        tokens = self.token_storage.load()
        if not tokens or "access_token" not in tokens:
            raise DataEngineerException(ErrorCode.OAUTH_NOT_AUTHENTICATED)
    ...
    def _refresh_tokens_unlocked(self) -> dict:
        ...
        if not tokens or "refresh_token" not in tokens:
            raise DataEngineerException(ErrorCode.OAUTH_NO_REFRESH_TOKEN)
        ...
        try:
            ...
            raise DataEngineerException(ErrorCode.OAUTH_TIMEOUT) from e
