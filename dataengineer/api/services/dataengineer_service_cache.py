"""Global LRU cache: project_id -> DataEngineerService.
...
class DataEngineerServiceCache:
    """Async LRU cache for DataEngineerService instances."""

    def __init__(self, max_size: int = 128):
        self._max_size = max_size
        self._cache: collections.OrderedDict[str, DataEngineerService] = collections.OrderedDict()
        self._futures: dict[str, asyncio.Future[DataEngineerService]] = {}
        self._lock = asyncio.Lock()

    async def get_or_create(
        self,
        project_id: str,
        factory: Callable[[], Awaitable[DataEngineerService]],
        expected_fingerprint: Optional[str] = None,
    ) -> DataEngineerService:
        """Return cached DataEngineerService or create via factory (thundering-herd safe).
        ...
        """
        is_creator = False
        stale_svc: Optional[DataEngineerService] = None
        ...
        async with self._lock:
            ...
            if project_id in self._cache:
                ...
                logger.info(f"Evicting DataEngineerService for project {project_id} due to AgentConfig fingerprint mismatch")
        ...
        for old_pid, old_svc in evicted:
            logger.info(f"LRU evicting DataEngineerService for project {old_pid}")
            asyncio.create_task(old_svc.shutdown())

        return svc

    async def evict(self, project_id: str) -> None:
        """Evict a DataEngineerService from cache (config change).
    ...
    async def _dispose(self, project_id: str, svc: DataEngineerService) -> None:
        """Shutdown a service, deferring if it still has active tasks."""
        if svc.has_active_tasks():
            logger.info(f"Evicting DataEngineerService for project {project_id} (deferring shutdown — active tasks)")
            asyncio.create_task(self._deferred_shutdown(project_id, svc))
        else:
            logger.info(f"Evicting DataEngineerService for project {project_id}")
            await svc.shutdown()

    @staticmethod
    async def _deferred_shutdown(project_id: str, svc: DataEngineerService) -> None:
    ...
    async def shutdown(self) -> None:
        """Shutdown all cached DataEngineerService instances (application exit)."""
        async with self._lock:
        ...
        for pid, svc in items:
            try:
                ...
                logger.exception(f"Error shutting down DataEngineerService for project {pid}")
        logger.info("DataEngineerServiceCache shut down")
