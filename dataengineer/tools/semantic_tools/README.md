...
This package provides a unified abstraction layer for semantic layer services (MetricFlow, dbt Semantic Layer, Cube, etc.), following the adapter pattern similar to `dataengineer/tools/db_tools/`.
...
```python
from dataengineer.tools.semantic_tools import BaseSemanticAdapter
...
```python
from dataengineer.tools.semantic_tools import semantic_adapter_registry
...
```python
from dataengineer.tools.semantic_tools import SemanticAdapterConfig  # Base config
```
...
```python
from dataengineer.tools.semantic_tools import SemanticStorageManager
...
- `{agent.home}/semantic_models/{datasource}/`
- `agent.home` is read from `agent.yml` config (defaults to `~/.dataengineer`)
...
def register():
    """Register MetricFlow adapter with DataEngineer semantic adapter registry."""
...
version = "0.1.0"
description = "MetricFlow adapter for DataEngineer semantic layer"
dependencies = [
    "pydantic>=2.0.0",
]
...
dataengineer-agent bootstrap-kb \
...
# Or let DataEngineer auto-create config from datasource
...
2. **Model path resolution**: `{agent.home}/semantic_models/{datasource}/`
   - `agent.home` is read from `agent.yml` (defaults to `~/.dataengineer`)
...
3. **Agent config lookup priority**:
   ...
   - `~/.dataengineer/conf/agent.yml` (home directory)

### Example agent.yml

```yaml
agent:
  home: /Users/myuser/.dataengineer  # Optional, defaults to ~/.dataengineer
...
- **Architecture Pattern**: [dataengineer/tools/db_tools/](../db_tools/) - Database adapter pattern
- **Storage Integration**: [dataengineer/storage/semantic_model/](../../storage/semantic_model/), [dataengineer/storage/metric/](../../storage/metric/)
- **Example Usage**: [dataengineer/agent/node/gen_metrics_agentic_node.py](../../agent/node/gen_metrics_agentic_node.py)
