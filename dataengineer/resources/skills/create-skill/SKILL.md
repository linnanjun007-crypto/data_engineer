...
- Check existing skills via `glob(pattern, path)` to avoid duplicates. The `~` expansion only applies to `path`, not `pattern`, so split the prefix out of the pattern: project-level `glob("*/SKILL.md", ".dataengineer/skills")`; user-level `glob("*/SKILL.md", "~/.dataengineer/skills")`.
...
2. **[Required]** Storage location — offer choices: project-level (`./.dataengineer/skills/`) or user-level (`~/.dataengineer/skills/`)
...
Use `write_file` from the filesystem tools. Paths must start with `.dataengineer/skills/` (project-level) or `~/.dataengineer/skills/` (user-level) — see Critical Rule #2:

```text
write_file(path=".dataengineer/skills/<skill-name>/SKILL.md", content=...)
```

If the user explicitly requested scripts, also create scripts/:
```text
write_file(path=".dataengineer/skills/<skill-name>/scripts/<script>.py", content=...)
```
...
- **Project-level** (`./.dataengineer/skills/`): version-controlled, project-specific
- **User-level** (`~/.dataengineer/skills/`): shared across projects
