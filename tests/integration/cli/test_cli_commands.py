...
def test_schema_linking(mock_args, capsys, schema_linking_input: List[Dict[str, Any]]):
    ...
    with patch("dataengineer.cli.repl.PromptSession.prompt") as mock_repl_prompt:
        mock_repl_prompt.side_effect = ["!sl", EOFError]

        with patch("dataengineer.cli.repl.DataEngineerCLI.prompt_input") as mock_internal_prompt:
...
def test_databases_command(mock_args, capsys):
    with patch("dataengineer.cli.repl.PromptSession.prompt") as mock_prompt:
...
def test_chat_command(mock_args, capsys, gen_sql_input: List[Dict[str, Any]]):
    ...
    with patch("dataengineer.cli.repl.PromptSession.prompt") as mock_prompt:
        ...
        with (
            patch("dataengineer.cli.repl.DataEngineerCLI.prompt_input") as mock_internal_prompt,
...
def test_save_command(mock_args, capsys):
    ...
    with patch("dataengineer.cli.repl.PromptSession.prompt") as mock_prompt:
        ...
        with (
            patch("dataengineer.cli.repl.DataEngineerCLI.prompt_input") as mock_internal_prompt,
            patch("dataengineer.cli.cli_context.CliContext.get_last_sql_context") as mock_context,
            patch("dataengineer.cli.agent_commands.OutputTool.execute") as mock_output,
        ):
            mock_internal_prompt.side_effect = [
                "json",  # file_type
                "/tmp",  # target_dir
...
class TestCLISearch:
    ...
    def test_search_document_command(self, mock_args, capsys):
        ...
        with patch("dataengineer.cli.repl.PromptSession.prompt") as mock_repl_prompt:
            ...
            with patch("dataengineer.cli.repl.DataEngineerCLI.prompt_input") as mock_internal:
