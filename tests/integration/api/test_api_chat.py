...
@pytest.fixture(scope="module")
def chat_dataengineer_service(chat_agent_config):
    from dataengineer.api.services.dataengineer_service import DataEngineerService

    return DataEngineerService(agent_config=chat_agent_config, project_id="chat_integration_test")


@pytest_asyncio.fixture(scope="module")
async def chat_client(chat_agent_config, chat_dataengineer_service):
    """AsyncClient wired to the full FastAPI app with real DataEngineerService."""
    import dataengineer.api.deps as deps_mod
    from dataengineer.api.auth import NoAuthProvider
    ...
    cache = DataEngineerServiceCache(max_size=4)

    async def _factory():
        return chat_dataengineer_service

    # Enter try/finally BEFORE mutating globals so failures during setup
    # (DataEngineerAPIService, init_deps, cache.get_or_create) still unwind the
    # patched module state and shut the cache down cleanly.
    try:
        mod.service = DataEngineerAPIService(agent_args)
...
class TestAPIChatN9:
    ...
    @pytest.mark.asyncio
    async def test_resume_from_cursor(self, chat_client, chat_dataengineer_service):
        """start task → wait for events → resume from cursor 0."""
        task = await _start_task(chat_dataengineer_service, "How many charter schools are there?", "resume_sess")
        try:
            ...
            await _cleanup_task(chat_dataengineer_service, task)
    ...
    @pytest.mark.asyncio
    async def test_stop_running_chat(self, chat_client, chat_dataengineer_service):
        """start a chat, stop it, verify task terminates."""
        task = await _start_task(
            chat_dataengineer_service,
    ...
    @pytest.mark.asyncio
    async def test_source_proxy_tool_result(self, chat_client, chat_dataengineer_service, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_ask_user_interaction(self, chat_client, chat_dataengineer_service):
