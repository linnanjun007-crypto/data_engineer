...
from dataengineer.utils.exceptions import DataEngineerException, ErrorCode, setup_exception_handler
...
class TestDataEngineerExceptionBuildMsg:
    """Tests for DataEngineerException.build_msg (line 167)."""

    def test_custom_message_takes_priority(self):
        ex = DataEngineerException(ErrorCode.COMMON_UNKNOWN, message="custom msg")
        assert "custom msg" in str(ex)

    def test_message_args_format_template(self):
        ex = DataEngineerException(
    ...
    def test_no_message_no_args_uses_desc(self):
        ex = DataEngineerException(ErrorCode.COMMON_UNKNOWN)
    ...
    def test_str_returns_message(self):
        ex = DataEngineerException(ErrorCode.COMMON_UNKNOWN, message="hello")
    ...
    def test_can_be_raised_and_caught(self):
        with pytest.raises(DataEngineerException) as exc_info:
            raise DataEngineerException(ErrorCode.COMMON_UNKNOWN, message="test raise")
        assert "test raise" in str(exc_info.value)

    def test_code_attribute_set(self):
        ex = DataEngineerException(ErrorCode.COMMON_FIELD_REQUIRED, message_args={"field_name": "f"})
        assert ex.code == ErrorCode.COMMON_FIELD_REQUIRED

    def test_message_args_attribute_set(self):
        ex = DataEngineerException(ErrorCode.COMMON_UNKNOWN, message_args={"key": "val"})
    ...
    def test_all_error_codes_instantiable(self, code):
        """Every ErrorCode can be used to create a DataEngineerException."""
        ex = DataEngineerException(code)
        assert isinstance(ex, DataEngineerException)
...
class TestSetupExceptionHandler:
    ...
    def test_handler_with_console_logger_datus_exception(self):
        """DataEngineerException invokes console_logger with formatted message."""
        original_hook = sys.excepthook
        console_logger = MagicMock()
        try:
            with patch("dataengineer.utils.exceptions.get_log_manager") as mock_lm:
                ...
                try:
                    raise DataEngineerException(ErrorCode.COMMON_UNKNOWN, message="test error")
    ...
    def test_prefix_wrap_func_applied(self):
        ...
        try:
            with patch("dataengineer.utils.exceptions.get_log_manager") as mock_lm:
                ...
                try:
                    raise DataEngineerException(ErrorCode.COMMON_UNKNOWN)
                except DataEngineerException:
                    exc_type, exc_val, tb = sys.exc_info()
                    sys.excepthook(exc_type, exc_val, tb)
    ...
    def test_non_debug_with_console_logger(self):
        ...
        try:
            with patch("dataengineer.utils.exceptions.get_log_manager") as mock_lm:
                ...
                setup_exception_handler(console_logger=console_logger)
                try:
                    raise DataEngineerException(ErrorCode.COMMON_UNKNOWN, message="non-debug test")
                except DataEngineerException:
                    exc_type, exc_val, tb = sys.exc_info()
                    sys.excepthook(exc_type, exc_val, tb)
