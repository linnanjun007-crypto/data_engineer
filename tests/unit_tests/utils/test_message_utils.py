...
def test_extract_user_input_json_decode_error_returns_fallback():
    ...
    with patch("dataengineer.utils.message_utils.is_structured_content", return_value=True):
        with patch("dataengineer.utils.message_utils.json.loads", side_effect=json.JSONDecodeError("err", "doc", 0)):
...
def test_extract_user_input_type_error_returns_fallback():
    ...
    raw = '[{"type": "user", "content": "hello"}]'
    with patch("dataengineer.utils.message_utils.is_structured_content", return_value=True):
        with patch("dataengineer.utils.message_utils.json.loads", side_effect=TypeError("bad type")):
            result = extract_user_input(raw)
