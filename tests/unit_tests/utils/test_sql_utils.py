...
from dataengineer.utils.sql_utils import (
    _fallback_sql_type,
    _first_statement,
    _is_escaped,
    _match_dollar_tag,
...
def test_parse_sql_type_starrocks_metadata_none_parse():
    ...
    with patch("dataengineer.utils.sql_utils.sqlglot.parse_one", return_value=None):
...
class TestParseSqlTypeExtended:
    ...
    def test_fallback_when_parse_exception(self):
        with patch("dataengineer.utils.sql_utils.sqlglot.parse_one", side_effect=Exception("parse error")):
...
class TestFormatSqlToPretty:
    ...
    def test_fallback_on_exception(self):
        sql = "SELECT * FROM t"
        with patch("dataengineer.utils.sql_utils.sqlglot.transpile", side_effect=Exception("parse error")):
            result = format_sql_to_pretty(sql, "duckdb")
        assert result == sql

    def test_fallback_on_empty_transpile_result(self):
        sql = "SELECT 1"
        with patch("dataengineer.utils.sql_utils.sqlglot.transpile", return_value=[]):
            result = format_sql_to_pretty(sql, "duckdb")
        assert result == sql
