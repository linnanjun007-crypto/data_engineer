...
from dataengineer.api.utils.path_utils import safe_resolve
...
class TestSafeResolveTraversalDetection:
    """Paths that escape base must raise DataEngineerException."""

    def test_double_dot_escapes_raises(self, tmp_path):
        """Path traversal with .. raises DataEngineerException."""
        with pytest.raises(DataEngineerException, match="escapes the project root"):
            safe_resolve(tmp_path, "../../etc/passwd")

    def test_absolute_path_outside_base_raises(self, tmp_path):
        """Absolute path outside base raises DataEngineerException."""
        with pytest.raises(DataEngineerException, match="escapes the project root"):
            safe_resolve(tmp_path, "/etc/passwd")

    def test_dot_dot_at_start_raises(self, tmp_path):
        """Single parent traversal raises DataEngineerException."""
        with pytest.raises(DataEngineerException, match="escapes the project root"):
