"""CI-level tests for dataengineer/api/routes/kb_routes.py.
...
from dataengineer.api.models.kb_models import BootstrapKbEvent
from dataengineer.api.routes.kb_routes import router
...
@pytest.fixture
def mock_dataengineer_service():
    """Create a mock DataEngineerService with agent_config and kb."""
    svc = MagicMock()
...
@pytest.fixture
def client(mock_dataengineer_service):
    """Create a TestClient with mocked dependencies."""
    from dataengineer.api.deps import get_dataengineer_service

    app = FastAPI()
    app.include_router(router)
    app.dependency_overrides[get_dataengineer_service] = lambda: mock_dataengineer_service
...
class TestBootstrapDocs:
    def test_bootstrap_docs_returns_sse_stream(self, client, mock_dataengineer_service):
        ...
        mock_dataengineer_service.kb.bootstrap_doc_stream = mock_stream
        mock_dataengineer_service.agent_config.document_configs = {"myplatform": MagicMock(type="github")}
    ...
    def test_bootstrap_docs_unknown_platform_no_source_returns_422(self, client, mock_dataengineer_service):
        """Unknown platform with no source → 422 because config is missing."""
        mock_dataengineer_service.agent_config.document_configs = {}
    ...
    def test_bootstrap_docs_known_platform_succeeds(self, client, mock_dataengineer_service):
        ...
        mock_dataengineer_service.agent_config.document_configs = {"snowflake": doc_cfg}
    ...
    def test_bootstrap_docs_local_path_traversal_returns_422(self, client, mock_dataengineer_service):
        """Local source with path traversal → safe_resolve raises DataEngineerException → 422."""
        # Platform must exist so we pass the first validation check
        doc_cfg = MagicMock()
        doc_cfg.type = "local"
        mock_dataengineer_service.agent_config.document_configs = {"myplatform": doc_cfg}

        with patch(
            "dataengineer.api.routes.kb_routes.safe_resolve",
            side_effect=DataEngineerException(
    ...
    def test_bootstrap_docs_local_source_from_config_validates_path(self, client, mock_dataengineer_service):
        """Local source type from config triggers path validation → DataEngineerException → 422."""
        doc_cfg = MagicMock()
        doc_cfg.type = "local"
        mock_dataengineer_service.agent_config.document_configs = {"testplatform": doc_cfg}
...
class TestCancelDocBootstrap:
    def test_cancel_doc_bootstrap_success(self, client):
        ...
        with patch("dataengineer.api.routes.kb_routes.cancel_stream", return_value=True):
    ...
    def test_cancel_doc_bootstrap_unknown_stream(self, client):
        ...
        with patch("dataengineer.api.routes.kb_routes.cancel_stream", return_value=False):
...
class TestBootstrapKb:
    ...
    def test_bootstrap_kb_returns_sse_stream(self, client, mock_dataengineer_service):
        ...
        mock_dataengineer_service.kb.bootstrap_stream = mock_stream
    ...
    def test_bootstrap_kb_path_validation_error(self, client, mock_dataengineer_service):
        """safe_resolve raises DataEngineerException for success_story path → 422."""
        with patch(
            "dataengineer.api.routes.kb_routes.safe_resolve",
            side_effect=DataEngineerException(
                ErrorCode.COMMON_VALIDATION_FAILED,
    ...
    def test_bootstrap_kb_sse_format_contains_id_and_event(self, client, mock_dataengineer_service):
