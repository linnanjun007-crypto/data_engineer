...
def test_config_exception(tmp_path):
    with pytest.raises(DataEngineerException, match="Agent configuration file not found: not_found.yml"):
        load_agent_config(config="not_found.yml", reload=True)

    with pytest.raises(
        DataEngineerException,
    ...
    with pytest.raises(DataEngineerException, match="Unsupported value `abc` for field `benchmark`"):
        agent_config.override_by_args(datasource="snowflake", benchmark="abc")

    with pytest.raises(DataEngineerException, match="Unsupported value `abc` for field `datasource`"):
...
def test_configuration_load(database: str, agent_config: AgentConfig):
    ...
    # ``{home}/data/{project_name}/dataengineer_db``. The name ``dataengineer_db`` is fixed;
    # project isolation happens via the parent directory.
    storage_path = agent_config.rag_storage_path()
    assert storage_path.endswith("dataengineer_db")
    assert f"/data/{agent_config.project_name}/dataengineer_db" in storage_path.replace(os.sep, "/")
    ...
    error_db = "abc"
    with pytest.raises(DataEngineerException, match=f"Unsupported value `{error_db}` for field `datasource`"):
        agent_config.current_datasource = error_db

    error_benchmark = "abc"
    with pytest.raises(DataEngineerException, match=f"Unsupported value `{error_benchmark}` for field `benchmark`"):
...
def test_benchmark_db_check(agent_config: AgentConfig):
    ...
    with pytest.raises(DataEngineerException, match="spider2 only support snowflake"):
        agent_config.override_by_args(
            **{
                "benchmark": "spider2",
                "datasource": db_name,
