...
"""
Unit tests for dataengineer/auth/claude_credential.py.
...
from dataengineer.auth.claude_credential import (
...
class TestGetClaudeSubscriptionToken:
    ...
    def _no_keychain(self):
        with patch("dataengineer.auth.claude_credential._read_keychain_credentials", return_value=None):
            yield
    ...
    def test_reads_credentials_file(self, tmp_path):
        ...
        with (
            ...
            patch("dataengineer.auth.claude_credential.Path.home", return_value=tmp_path),
        ):
            token, source = get_claude_subscription_token(api_key_from_config=None)
    ...
    def test_raises_when_not_found(self, tmp_path):
        """Raises DataEngineerException when no token source is available."""
        with (
            patch.dict("os.environ", {}, clear=True),
            patch("dataengineer.auth.claude_credential.Path.home", return_value=tmp_path),
        ):
    ...
    def test_missing_placeholder_without_fallback_raises(self, tmp_path):
        """<MISSING:...> placeholder with no env/file raises DataEngineerException."""
        with (
            patch.dict("os.environ", {}, clear=True),
            patch("dataengineer.auth.claude_credential.Path.home", return_value=tmp_path),
        ):
    ...
    def test_unresolved_env_placeholder_without_fallback_raises(self, tmp_path):
        """${VAR} placeholder with no env/file raises DataEngineerException."""
        with (
            patch.dict("os.environ", {}, clear=True),
            patch("dataengineer.auth.claude_credential.Path.home", return_value=tmp_path),
        ):
...
class TestReadKeychainCredentials:
    def test_returns_none_on_non_darwin(self):
        with patch("dataengineer.auth.claude_credential.sys") as mock_sys:
    ...
    def test_returns_parsed_json_on_success(self):
        ...
        with (
            patch("dataengineer.auth.claude_credential.sys") as mock_sys,
            patch("dataengineer.auth.claude_credential.subprocess.run", return_value=mock_result),
        ):
    ...
    def test_returns_none_on_timeout(self):
        with (
            patch("dataengineer.auth.claude_credential.sys") as mock_sys,
            patch(
                "dataengineer.auth.claude_credential.subprocess.run",
    ...
    def test_returns_none_when_security_not_found(self):
        with (
            patch("dataengineer.auth.claude_credential.sys") as mock_sys,
            patch("dataengineer.auth.claude_credential.subprocess.run", side_effect=FileNotFoundError),
        ):
            mock_sys.platform = "darwin"
            assert _read_keychain_credentials() is None
...
class TestKeychainIntegration:
    ...
    def test_env_var_takes_priority_over_keychain(self):
        with (
            patch.dict("os.environ", {"CLAUDE_CODE_OAUTH_TOKEN": "sk-ant-oat01-env"}),
            patch("dataengineer.auth.claude_credential.subprocess.run") as mock_run,
        ):
            token, source = get_claude_subscription_token(api_key_from_config=None)
            assert token == "sk-ant-oat01-env"
            assert "CLAUDE_CODE_OAUTH_TOKEN" in source
