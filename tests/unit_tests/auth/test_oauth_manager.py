...
from dataengineer.auth.token_storage import TokenStorage
...
class TestGetAccessToken:
    def test_raises_when_not_authenticated(self, manager):
        with pytest.raises(DataEngineerException, match="Not authenticated"):
...
class TestRefreshTokens:
    @patch("dataengineer.auth.oauth_manager.httpx.post")
    ...
    def test_raises_without_refresh_token(self, manager):
        ...
        with pytest.raises(DataEngineerException, match="No refresh token"):
            manager.refresh_tokens()


class TestLoginBrowser:
    @patch("dataengineer.auth.oauth_manager.webbrowser.open")
    @patch("dataengineer.auth.oauth_manager.HTTPServer")
...
class TestLoginBrowserFull:
    ...
    @patch("dataengineer.auth.oauth_manager.HTTPServer")
    @patch.object(OAuthManager, "_exchange_code")
    @patch("dataengineer.auth.oauth_manager.generate_pkce_pair")
    @patch("dataengineer.auth.oauth_manager.generate_state")
    ...
    @patch("dataengineer.auth.oauth_manager.time.monotonic")
    @patch("dataengineer.auth.oauth_manager.webbrowser.open")
    @patch("dataengineer.auth.oauth_manager.HTTPServer")
    @patch("dataengineer.auth.oauth_manager.generate_pkce_pair")
    @patch("dataengineer.auth.oauth_manager.generate_state")
    def test_browser_flow_error(self, mock_state, mock_pkce, mock_server_cls, mock_browser, mock_monotonic, manager):
        ...
        with pytest.raises(DataEngineerException, match="No authorization code"):
...
class TestRefreshTokensErrors:
    ...
    def test_http_error_raises_datus_exception(self, mock_post, manager):
        ...
        with pytest.raises(DataEngineerException, match="Token refresh failed"):
            manager.refresh_tokens()
    ...
    def test_timeout_raises_datus_exception(self, mock_post, manager):
        ...
        with pytest.raises(DataEngineerException, match="timed out"):
            manager.refresh_tokens()
...
class TestExchangeCodeErrors:
    ...
    def test_http_error_raises_datus_exception(self, mock_post, manager):
        ...
        with pytest.raises(DataEngineerException, match="Code exchange failed"):
...
class TestHttpTimeout:
    ...
    def test_refresh_passes_timeout(self, mock_post, manager):
        from dataengineer.auth.oauth_config import HTTP_TIMEOUT
