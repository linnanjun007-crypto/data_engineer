...
class TestDBFuncToolCompressorModelName:
    ...
    def test_compressor_uses_agent_config_model(self):
        ...
        with (
            patch("dataengineer.tools.func_tool.database.SchemaWithValueRAG") as mock_rag,
            patch("dataengineer.tools.func_tool.database.SemanticModelRAG") as mock_sem,
        ):
    ...
    def test_compressor_defaults_without_agent_config(self):
        ...
        with (
            patch("dataengineer.tools.func_tool.database.SchemaWithValueRAG"),
            patch("dataengineer.tools.func_tool.database.SemanticModelRAG"),
        ):
...
class TestGetConnectorRouting:
    ...
    def test_explicit_database_raises_when_not_configured(self):
        """When caller explicitly passes a database that doesn't exist, raise DataEngineerException (no silent fallback)."""
        from dataengineer.tools.db_tools.db_manager import DBManager
        ...
        with pytest.raises(DataEngineerException, match="not configured"):
