...
from dataengineer.tools.func_tool.base import FuncToolResult, normalize_null
from dataengineer.tools.func_tool.semantic_tools import _run_async
from dataengineer.tools.semantic_tools.models import QueryResult
...
def semantic_tools():
    ...
    with (
        patch("dataengineer.tools.func_tool.semantic_tools.SemanticModelRAG"),
        patch("dataengineer.tools.func_tool.semantic_tools.MetricRAG"),
    ):
...
class TestQueryMetricsCompression:
    ...
    def test_query_metrics_success_with_compression(self, semantic_tools, mock_adapter):
        ...
        with patch("dataengineer.tools.func_tool.semantic_tools._run_async", return_value=query_result):
    ...
    def test_query_metrics_adapter_exception(self, semantic_tools):
        ...
        with patch(
            "dataengineer.tools.func_tool.semantic_tools._run_async",
...
class TestRunAsync:
    def test_delegates_to_run_async_utility(self):
        mock_coro = Mock()
        with patch("dataengineer.utils.async_utils.run_async", return_value="result") as mock_run:
...
class TestAvailableTools:
    def test_no_adapter_returns_three_tools(self, semantic_tools_ext):
        with patch("dataengineer.tools.func_tool.semantic_tools.trans_to_function_tool") as mock_trans:
...
class TestListMetrics:
    ...
    def test_falls_back_to_adapter(self, semantic_tools_with_adapter):
        ...
        with patch("dataengineer.tools.func_tool.semantic_tools._run_async", return_value=[mock_metric]):
...
class TestGetDimensions:
    def test_with_adapter(self, semantic_tools_with_adapter):
        tool, mock_adapter = semantic_tools_with_adapter
        with patch("dataengineer.tools.func_tool.semantic_tools._run_async", return_value=["date", "region"]):
...
class TestValidateSemantic:
    ...
    def test_valid_result(self, semantic_tools_with_adapter):
        ...
        with patch("dataengineer.tools.func_tool.semantic_tools._run_async", return_value=mock_validation):
    ...
    def test_exception_returns_failure(self, semantic_tools_with_adapter):
        ...
        with patch("dataengineer.tools.func_tool.semantic_tools._run_async", side_effect=Exception("adapter crash")):
            result = tool.validate_semantic()
...
class TestAttributionAnalyze:
    ...
    def test_success_with_dict_anomaly_context(self, semantic_tools_with_adapter):
        ...
        with patch("dataengineer.tools.func_tool.semantic_tools._run_async", return_value=mock_result):
    ...
    def test_exception_returns_failure(self, semantic_tools_with_adapter):
        ...
        with patch("dataengineer.tools.func_tool.semantic_tools._run_async", side_effect=Exception("analysis failed")):
            result = tool.attribution_analyze(
                metric_name="revenue",
                candidate_dimensions=["region"],
                baseline_start="2024-01-01",
...
class TestReloadAdapter:
    ...
    def test_reload_adapter_fails_returns_false(self, semantic_tools_with_adapter):
        ...
        with patch("dataengineer.tools.func_tool.semantic_tools.semantic_adapter_registry") as mock_registry:
