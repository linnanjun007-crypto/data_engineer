...
from dataengineer.cli.datasource_app import DatasourceApp, DatasourceSelection, _View
...
class TestTestConnectivity:
    def test_success(self):
        cli = _make_cli()
        cmds = DatasourceCommands(cli)
        with patch("dataengineer.cli.datasource_commands.detect_db_connectivity", return_value=(True, "")):
            assert cmds._test_connectivity("test", {"type": "duckdb"}) is True

    def test_failure(self):
        cli = _make_cli()
        cmds = DatasourceCommands(cli)
        with patch("dataengineer.cli.datasource_commands.detect_db_connectivity", return_value=(False, "refused")):
...
class TestSave:
    def test_save_calls_configuration_manager(self):
        cli = _make_cli()
        cmds = DatasourceCommands(cli)
        with patch("dataengineer.configuration.agent_config_loader.configuration_manager") as mock_mgr:
    ...
    def test_save_returns_false_on_exception(self):
        cli = _make_cli()
        cmds = DatasourceCommands(cli)
        with patch("dataengineer.configuration.agent_config_loader.configuration_manager", side_effect=Exception("boom")):
            result = cmds._save()
            assert result is False
...
class TestDatasourceAppViews:
    ...
    def test_enter_type_select(self):
        ...
        app = DatasourceApp(cli.agent_config, MagicMock())
        with patch(
            "dataengineer.tools.db_tools.connector_registry.list_available_adapters",
