...
from dataengineer.cli.web.config_manager import get_available_datasources


def test_get_available_datasources_from_wrapped_agent_config() -> None:
    config = {"agent": {"services": {"datasources": {"duckdb": {}, "snowflake": {}}}}}

    with patch("dataengineer.cli.web.config_manager._load_config_cached", return_value=config):
...
def test_get_available_datasources_returns_empty_on_missing_file() -> None:
    with (
        patch("dataengineer.cli.web.config_manager._load_config_cached", side_effect=FileNotFoundError("missing")),
        patch("dataengineer.cli.web.config_manager.logger") as mock_logger,
    ):
...
def test_get_available_datasources_returns_empty_on_parse_error() -> None:
    with (
        patch("dataengineer.cli.web.config_manager._load_config_cached", side_effect=ValueError("bad yaml")),
        patch("dataengineer.cli.web.config_manager.logger") as mock_logger,
    ):
